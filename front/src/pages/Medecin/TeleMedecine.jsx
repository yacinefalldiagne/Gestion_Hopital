...
function TeleMedecine() {
  ...
  const handleSubmit = async (e) => {
    ...
    for (const item of selectedFiles) {
      if (item.type === 'report') {
        try {
          const pdfBlob = generateReportPDF(item, patientData);
          formData.append('reports', pdfBlob, `report_${item._id}.pdf`);
        } catch (pdfErr) {
          console.error('PDF generation error:', pdfErr);
          setError('Erreur lors de la génération du PDF du rapport.');
          return;
        }
      } else if (item.type === 'dicom') {
        const previewUrl = `/instances/${item.instanceId}/preview`;
        formData.append('dicomUrls', previewUrl);
      }
    }
    ...
    try {
      ...
      setError('Erreur lors de l’envoi des données: ' + (err.response?.data?.message || err.message));
    }
  };
  ...
  const selectConsultation = async (consultationId) => {
    ...
    try {
      ...
      console.error('Fetch messages error:', err.response?.status, err.response?.data);
